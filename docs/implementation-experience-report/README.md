...
ambiguous types. The mappings that do intersect now accurately reflect the
horizontal relationships between the taxon concepts (I can see that).
...
revision (where the concepts of higher taxa come from) regognises three taxa and
another one in the bottom-raight corner in which two taxa are recognised. There
